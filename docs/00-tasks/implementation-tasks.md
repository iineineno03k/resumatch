...
### 5.1 PDF テキスト抽出モジュール
- [x] `pdfjs-dist` インストール（legacy build for Node.js）
...
### 5.5 テストについて
- 単体テストは削除済み（ライブラリ互換性テストにしかならず価値が低いため）
- 実際の履歴書での動作確認は E2E テスト（Phase 7）で実施
...
**6.4.2 フロー実装（完了）**
- [x] チーム未所属時のリダイレクト処理（(team-required) グループで実装）
- [x] 作成後のダッシュボードへの遷移
...
- [x] `/jobs` - 求人カード一覧
- [x] 新規作成ボタン
- [x] 求人カードコンポーネント（タイトル、ステータス、応募者数）
- [x] **レビュー:** Storybook で確認

**6.6.2 求人作成・編集画面（完了）**
- [x] `/jobs/new` - 求人作成フォーム
- [x] `/jobs/[id]/edit` - 求人編集フォーム
- [x] `/jobs/[id]` - 求人詳細ページ（編集リンク含む）
- [x] フォームフィールド（タイトル、説明、要件、ステータス）
- [x] **レビュー:** Storybook で確認

**6.6.3 フロー実装（完了）**
- [x] Server Actions 連携（createJob, updateJob）
- [x] 作成/編集後のリダイレクト
...
**6.7.1 一覧UI（完了）**
- [x] `/applicants` - 応募者テーブル/カード一覧
- [x] 各行: 名前、応募求人、ステータス、評価、日付
- [x] **レビュー:** Storybook で確認
...
**6.8.1 登録フォーム**
- [x] `/applicants/new` - 応募者情報入力フォーム
- [x] フィールド: 氏名、メール、電話、応募求人
- [x] 履歴書アップロード（FileUpload 使用）
- [x] **レビュー:** Playwright で動作確認

**6.8.2 フロー実装**
- [x] API 連携（応募者作成、履歴書アップロード）
- [ ] AI解析の自動実行（オプション）※Phase 6.9で実装

---

### 6.9 応募者詳細画面（メイン画面）（完了）

**6.9.1 基本情報セクション**
- [x] `/applicants/[id]` - 応募者詳細レイアウト
- [x] ヘッダー: 名前、ステータス、評価
- [x] 基本情報: メール、電話、応募求人、応募日
- [x] **レビュー:** Playwright で動作確認

**6.9.2 AI解析結果セクション**
- [x] 解析結果カード（スキル、経験年数、学歴等）
- [x] 解析中/未解析の状態表示
- [x] 再解析ボタン
- [x] **レビュー:** Playwright で動作確認
...
**6.9.3 履歴書セクション**
- [x] PDFダウンロードリンク
- [x] AI解析結果に統合（履歴書なし/未解析/解析済みの状態表示）
- [x] **レビュー:** Playwright で動作確認
...
**6.9.5 フロー実装**
- [x] API 連携（詳細取得、メモ追加、ステータス更新、AI解析）
...
4. **AI/PDF処理の単体テストは不要** - ライブラリ互換性テストにしかならない。E2Eで確認する
